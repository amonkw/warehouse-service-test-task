...
class KafkaMessageData(BaseModel):
    ...
    movement_id: UUID = Field(
        ..., description="Уникальный ID перемещения (одинаковый для отправки/приемки)"
    )
    ...
    event: MovementEventType = Field(
        ..., description="Тип события: arrival или departure"
    )
...
class KafkaFullMessage(BaseModel):
    ...
    source: str = Field(
        default=None,
        pattern=r"^WH-\d{4}$",
        description="Источник отправки в формате WH-XXXX",
    )
    specversion: Optional[str] = Field(
        default=None, description="Версия спецификации CloudEvents"
    )
    ...
    datacontenttype: Optional[str] = Field(
        default="application/json", description="Тип содержимого"
    )
    dataschema: Optional[str] = Field(default=None, description="Схема данных")
    time: Optional[int] = Field(
        default=None, description="Временная метка в миллисекундах"
    )
